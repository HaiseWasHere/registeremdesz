...
if(member.id === message.author.id) return message.channel.send('Kendini Kayıt Edemezsin Dostum.').then(x => x.delete({timeout: 5000}));

if(member.id === message.guild.OwnerID) return message.channel.send("Sunucu Sahibini Kayıt Edemezsin Dostum.").then(x => x.delete({timeout: 5000}));

if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send("Kullanıcı Senle Aynı Yada Senden Üst Pozisyonda!").then(x => x.delete({timeout: 5000}));
...
let mesaj = `**Kullanıcı Başarıyla Kayıt Edildi, Vip Rolu Verildi <@&${set.roller.erkek}> ve <@&${set.roller.erkek2}> Rolleri Verildi**`;
client.channels.cache.get(set.kanallar.kayıtkanal).send(mesaj)


let mesajj = `**Kayıt Başarılı** \n**Kayıt Eden Yetkili** : ${message.author} \n**Verilen Rol** : <@&${set.roller.erkek}> , <@&${set.roller.erkek2}> , <@&${set.roller.vip}> \n**Alınan Rol**: <@&${set.roller.unregister}>`

client.channels.cache.get(set.kanallar.log).send(mesajj)
